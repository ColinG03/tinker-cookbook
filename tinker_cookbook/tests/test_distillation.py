...
async def _test_thinking_penalty_applied_when_over_threshold_async():
    """Verify advantage positions inside <think> are penalized when token count exceeds threshold."""
    tokenizer = get_tokenizer("Qwen/Qwen3-8B")

    # Build a sequence with a thinking block
    text = "Hello <think>some reasoning content here</think> answer"
    tokens = tokenizer.encode(text, add_special_tokens=False)
    model_input = tinker.ModelInput.from_ints(tokens[:-1])
    target_tokens = tokens[1:]
    ...
    datum = tinker.Datum(
        ...
        loss_fn_inputs={
            "target_tokens": tinker.TensorData.from_torch(torch.tensor(target_tokens)),
            "logprobs": tinker.TensorData.from_torch(torch.zeros(len(target_tokens))),
            "mask": tinker.TensorData.from_torch(torch.ones(len(target_tokens))),
            "advantages": tinker.TensorData.from_torch(torch.ones(len(target_tokens))),
        },
    )

    # Count how many thinking tokens this sequence has
    full_seq = model_input.append_int(target_tokens[-1])
    reasoning_mask_full = identify_reasoning_tokens(full_seq, tokenizer)
    reasoning_mask_target = reasoning_mask_full[1: len(target_tokens) + 1]
    thinking_count = int(reasoning_mask_target.sum().item())
    assert thinking_count > 0, "Test sequence must contain thinking tokens"

    # Set threshold well below the actual count so penalty triggers
    threshold = max(1, thinking_count - 3)

    teacher_client = MagicMock()
    teacher_logprobs = torch.zeros(len(full_seq.to_ints())).tolist()
    teacher_client.compute_logprobs_async = AsyncMock(return_value=teacher_logprobs)
    ...
    metrics = await incorporate_kl_penalty(
        ...
        reasoning_kl_multiplier=1.0,
        tokenizer=tokenizer,
        thinking_penalty_threshold=threshold,
        thinking_penalty_per_token=per_token,
    )

    updated_advantages = datum.loss_fn_inputs["advantages"].to_torch()
    expected_penalty = (thinking_count - threshold) * per_token

    # Reasoning positions should have been reduced by the penalty
    reasoning_advantages = updated_advantages[reasoning_mask_target]
    assert torch.allclose(reasoning_advantages, torch.full_like(reasoning_advantages, 1.0 - expected_penalty)), \
        f"Reasoning advantages should be 1.0 - {expected_penalty}, got {reasoning_advantages}"

    # Non-reasoning positions should remain at 1.0 (only KL with zeros changes nothing)
    non_reasoning_advantages = updated_advantages[~reasoning_mask_target]
    assert torch.allclose(non_reasoning_advantages, torch.ones_like(non_reasoning_advantages)), \
        "Non-reasoning advantages should be unchanged"

    # Check metrics
    assert "thinking_tokens_per_turn" in metrics
    assert metrics["thinking_tokens_per_turn"] == thinking_count
    assert "thinking_length_penalty" in metrics
    assert metrics["thinking_length_penalty"] == expected_penalty


def test_thinking_penalty_applied_when_over_threshold():
    asyncio.run(_test_thinking_penalty_applied_when_over_threshold_async())


async def _test_thinking_penalty_not_applied_below_threshold_async():
    """No penalty when thinking token count is at or below the threshold."""
    tokenizer = get_tokenizer("Qwen/Qwen3-8B")

    text = "Hello <think>short</think> answer"
    tokens = tokenizer.encode(text, add_special_tokens=False)
    model_input = tinker.ModelInput.from_ints(tokens[:-1])
    target_tokens = tokens[1:]
    ...
    # Set threshold above the actual thinking count
    threshold = thinking_count + 100
    ...
    metrics = await incorporate_kl_penalty(
        ...
        tokenizer=tokenizer,
        thinking_penalty_threshold=threshold,
        thinking_penalty_per_token=0.5,
    )

    # All advantages should remain at 1.0 (KL is zero, no penalty applied)
    updated = datum.loss_fn_inputs["advantages"].to_torch()
    assert torch.allclose(updated, torch.ones_like(updated)), \
        "Advantages should be unchanged when thinking count <= threshold"

    assert metrics["thinking_length_penalty"] == 0.0


def test_thinking_penalty_not_applied_below_threshold():
    asyncio.run(_test_thinking_penalty_not_applied_below_threshold_async())


async def _test_thinking_penalty_defaults_noop_async():
    """With default config (threshold=0, per_token=0.0), no penalty is applied."""
    tokenizer = get_tokenizer("Qwen/Qwen3-8B")

    text = "Hello <think>lots of reasoning content here that is quite long</think> answer"
    tokens = tokenizer.encode(text, add_special_tokens=False)
    model_input = tinker.ModelInput.from_ints(tokens[:-1])
    target_tokens = tokens[1:]
    ...
    metrics = await incorporate_kl_penalty(
        ...
        reasoning_kl_multiplier=1.0,
        tokenizer=tokenizer,
        # Defaults: threshold=0, per_token=0.0
    )

    updated = datum.loss_fn_inputs["advantages"].to_torch()
    assert torch.allclose(updated, torch.ones_like(updated)), \
        "Default config should not apply any thinking penalty"

    assert metrics["thinking_length_penalty"] == 0.0
    assert metrics["thinking_tokens_per_turn"] > 0, "Should still count thinking tokens"


def test_thinking_penalty_defaults_noop():
    asyncio.run(_test_thinking_penalty_defaults_noop_async())
